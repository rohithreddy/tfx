...
def _PartitionFn(record: bytes, num_partitions: int, buckets: List[int]) -> int:
  assert num_partitions == len(buckets)
...
class BaseExampleGenExecutor(
  ...
  def Do(self, input_dict: Dict[Text, List[types.TfxType]],
    ...
    with beam.Pipeline(argv=self._get_beam_pipeline_args()) as pipeline:
      ...
      split_names = utils.generate_output_split_names(input_config,
                                                      output_config)

      example_splits = []
      if output_config.split_config.splits:
        # Use output splits, input must have only one split.
        assert len(input_config.splits) == 1
      ...
      for index, example_split in enumerate(example_splits):
        (example_split
         | 'WriteSplit' + split_names[index] >> _WriteSplit(
             types.get_split_uri(output_dict['examples'], split_names[index])))
